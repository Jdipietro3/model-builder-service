...
class Run(Base):
    ...
    # Set only when this run's plan is an LLM-proposed revision of an earlier run's
    # plan on the same data (vs. parent_run_id, which means "retrained on newer data").
    revision_of_run_id: Mapped[str | None] = mapped_column(String(32), nullable=True)
