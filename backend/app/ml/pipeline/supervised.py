...
This is the deterministic core that used to live inline in ``training.run_plan``.
Behavior is unchanged — same feature selection, same preprocessing pipeline, same
progress beats, same rounding — it has only been lifted behind the ``TaskRunner``
contract so other task families can register alongside it.
"""
